...
local acpi_battery_percent = function(battery)
    local now = io.open("/sys/class/power_supply/" .. battery .. "/energy_now")
        or io.open("/sys/class/power_supply/" .. battery .. "/charge_now")
    local full = io.open("/sys/class/power_supply/" .. battery .. "/energy_full")
        or io.open("/sys/class/power_supply/" .. battery .. "/charge_full")
    if (now == nil) or (full == nil) then
        return 0
    end
